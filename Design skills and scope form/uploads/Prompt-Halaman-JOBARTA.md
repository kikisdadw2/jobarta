...
Target state: halaman "/" publik yang menjelaskan JOBARTA dan mengarahkan ke peta & registrasi.

Baca dulu: src/ (struktur routing & komponen layout yang sudah ada), JOBARTA Home.html (referensi visual jika ada).

Bangun:
- Hero: judul "Cari kerja terdekat lewat peta", subjudul singkat, 2 CTA — "Lihat Peta Lowongan" (ke /peta) dan "Daftar" (ke /register).
...
Selesai jika: kedua halaman live & ter-link dari footer dan checkbox consent di /register.
```
